...
-- Add recent_orders to get_public_race_dash so the live "Order Terkini" feed
-- actually shows orders placed today (from public.customers, i.e. /order page).
-- Re-defines the function with the new field. Safe to re-run (CREATE OR REPLACE).
...
BEGIN
  ...
  -- Per-material aggregation (unchanged)
  WITH known(name) AS (

  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Calculate sums
        const yearlyRevenue = yearData.reduce((sum, item) => sum + (parseFloat(item.sales_amount) || 0), 0);
        const yearlyProfit = yearData.reduce((sum, item) => sum + (parseFloat(item.gross_profit) || 0), 0);
        
        const todayRevenue = todayData.reduce((sum, item) => sum + (parseFloat(item.sales_amount) || 0), 0);
        const todayProfit = todayData.reduce((sum, item) => sum + (parseFloat(item.gross_profit) || 0), 0);
        
        const yesterdayRevenue = yesterdayData.reduce((sum, item) => sum + (parseFloat(item.sales_amount) || 0), 0);
        const yesterdayProfit = yesterdayData.reduce((sum, item) => sum + (parseFloat(item.gross_profit) || 0), 0);
        
        const monthlyRevenue = monthData.reduce((sum, item) => sum + (parseFloat(item.sales_amount) || 0), 0);
        const monthlyProfit = monthData.reduce((sum, item) => sum + (parseFloat(item.gross_profit) || 0), 0);
    ...
    const generateDailyData = async (month: number, year: number) => {
      try {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
          ...
          const dayRevenue = data.reduce((sum, item) => sum + (parseFloat(item.sales_amount) || 0), 0);
          const dayProfit = data.reduce((sum, item) => sum + (parseFloat(item.gross_profit) || 0), 0);
